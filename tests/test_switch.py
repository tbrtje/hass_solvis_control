...
from custom_components.solvis_leo.switch import SolvisSwitch, async_setup_entry, _LOGGER
...
async def test_handle_coordinator_update_valid(mock_solvis_switch):
    ...
    with patch("custom_components.solvis_leo.entity.process_coordinator_data", return_value=(True, 1, {"raw_value": 1})) as proc_patch:
...
async def test_async_turn_on_success(mock_solvis_switch):
    ...
    mock_solvis_switch.hass = MagicMock()
    with patch("custom_components.solvis_leo.switch.write_modbus_value", new=AsyncMock(return_value=True)) as write_patch:
...
async def test_async_turn_on_failure(mock_solvis_switch):
    ...
    mock_solvis_switch.hass = MagicMock()
    with patch("custom_components.solvis_leo.switch.write_modbus_value", new=AsyncMock(return_value=False)) as write_patch:
        with patch("custom_components.solvis_leo.switch._LOGGER.error") as log_error:
            with patch.object(mock_solvis_switch, "async_write_ha_state") as write_state_patch:
                await mock_solvis_switch.async_turn_on()
                write_patch.assert_awaited_once_with(mock_solvis_switch.coordinator.modbus, 1, 1)
